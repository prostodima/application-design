...
## Пояснения к решению
Удалена сущность `RoomAvailability`. Вместо этого предлагается проверять доступность номера через `Order` напрямую. Такой подход более эффективен, так как нам не надо генерировать записи в `RoomAvailability` для всех дней и всех номеров. Плюс, в рамках задания позволяет упростить решение и не связываться с транзакционностью для поддержания консистентности между заказами и доступными номерами.

Все решения сделаны на уровне прототипа: оставлены несколько TODO, тестые покрывают только базовый сценарий.
