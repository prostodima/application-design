...
## Пояснения к решению
Удалена сущность `RoomAvailability`. Вместо этого предлагается проверять доступность номера через `Order` напрямую. Такой подход более эффективен, так как нам не надо генерировать записи в `RoomAvailability` для всех дней и всех номера. Плюс, в рамках задания позволяет упростить решение и не связываться с транзакционностью для поддержания консистентности между заказами и доступными номерами.
